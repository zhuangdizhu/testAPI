...
void build_scheduler_context(
    ...
    strcpy(scheduler_ctx->job_id, uuid);
    strcpy(scheduler_ctx->server_host, dest_node);
    return;
}
...
int request_to_scheduler(void *acc_ctx) {
    ...
    char *job_id = scheduler_ctx->job_id;
    char *acc_name = acc_context->acc_name;
    char *server_host = scheduler_ctx->server_host;
    ...
    memcpy(send_ctx.job_id,job_id,strlen(job_id));
    memcpy(send_ctx.server_host,server_host,strlen(server_host));
