...
{	
	char *acc_name, *host, *uuid, *dest_node;
	...
	if (argc != 7) {
		printf("Usage : ./test_bench <acc_name> <workload> <uuid> <dest_node> <scheduler host> <scheduler port>\n");
		printf("\tworkload must be a mutiple of 4 K bytes.\n");
		printf("\tExample : ./test_bench AES 1024 1 192.168.8.1 tian01 9000(means executing AES with workloads of 1024 * 4K bytes, UUID = 1\n");
		return -1;
   	}
