...
debug = 0
BUFFER_SIZE = 1024
recv_fmt = "!16s16s16s16s16s16s16s"
...
class FpgaScheduler(object):
    ...
    def execute_scheduling(self, job_id, job_node_ip, server_host, event_type):
        ...
        self.conduct_fake_scheduling(job_id, server_host, event_type)
    ...
    def conduct_fake_scheduling(self, job_id, server_host, event_type):
        if event_type == "JOB_ARRIVAL":
            self.job_list[job_id].default_server_host = server_host
            section_id = self.pick_idle_section(server_host)
            if section_id != None:
                self.trigger_new_job(job_id, section_id)
                return
            ...
            if len(self.job_waiting_list):
                for curr_job_id in self.job_waiting_list:
                    if self.job_list[curr_job_id].default_server_host == server_host:
                        self.trigger_new_job(curr_job_id, section_id)
                        self.remove_job_from_wait_queue(curr_job_id)
                        return
    ...
    def handle_open_request(self, client_addr, data):
        ...
        self.execute_scheduling(current_job_id, job_node_ip, server_host, "JOB_ARRIVAL")
    ...
    def handle_close_request(self, data):
        ...
        section_id = self.job_list[job_id].job_current_section_id
        server_host = self.section_list[section_id].node_ip
        self.execute_scheduling(job_id, job_node_ip, server_host, "JOB_COMPLETE")
        interval = time.time()-self.job_list[job_id].job_arrival_time
