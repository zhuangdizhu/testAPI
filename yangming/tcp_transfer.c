...
size_t send_msg(int fd, void *buffer, size_t len, size_t chunk, double max_bps)
{
    ...
    while(sent_block < len) {
        ...
        if (max_bps > 0){
            ideal_sec = ceil((double)1000000*chunk / max_bps);  
...
void * tcp_server_data_transfer(void * server_param) {
    ...
    char *send_buff = (char *)memset(malloc(tmp_size),0, tmp_size);
    ...
    while (1) {
        ...
        memcpy(send_buff, "ready", sizeof("ready"));
        
        if (( send_size = send(rqst_fd, send_buff, tmp_size, 0) ) < 0){
            printf("fail to send\n");
            exit(1);
        }
